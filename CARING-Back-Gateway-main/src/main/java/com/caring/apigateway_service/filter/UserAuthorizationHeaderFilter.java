...
public class UserAuthorizationHeaderFilter extends AbstractGatewayFilterFactory<UserAuthorizationHeaderFilter.Config> {
    ...
    public UserAuthorizationHeaderFilter(Environment environment) {
        super(Config.class);
    ...
    public GatewayFilter apply(Config config) {
        return ((exchange, chain) -> {
            ...
            MemberInfo memberInfo = TokenUtil.parseJwt(jwt, env.getProperty("token.secret-user"));
            if (memberInfo.getMemberCode() == null) {
                return onError(exchange, "Invalid USER JWT token", HttpStatus.UNAUTHORIZED);
            }
